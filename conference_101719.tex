...
% ============================================================
% I. INTRODUCTION - [Change 2,4] Stronger critique + elevated insights
% ============================================================
...
% ============================================================
% II. BACKGROUND AND RELATED WORK - [Change 5] Added critical gap analysis
% ============================================================
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\columnwidth, trim=0 105 0 240, clip]{Fig0_Architecture_v2.pdf}
...
The Llama-3.2-1B model is serialized into a custom binary format. The file begins with a 28-byte header encoding the model configuration (dimensions, layer count, head count, vocabulary size), followed by contiguous weight data. We use memory-mapped I/O (\texttt{CreateFileMapping} on Windows) to map the entire file into the host address space without copying, enabling zero-copy pointer arithmetic to locate each weight tensor.
...
% ============================================================
% IV. MICRO-ARCHITECTURE OPTIMIZATION AND ABLATION
% [Change 4] Each subsection now leads with a "Finding"
% ============================================================
...
\begin{table}[htbp]
\centering
\caption{Nsight Compute Profiling of the GEMV Kernel Across Optimization Stages (RTX 4060, $d=2048$)}
\label{tab:nsight}
...
% [Change 6] Fixed ~1250 to precise 1252
\begin{table}[htbp]
\centering
...
\midrule
\textbf{MiniLlama (Ours)} & \textbf{INT8+FP32$^{\mathrm{b}}$} & \textbf{2054} & \textbf{35.2} & \textbf{11.52} \\
\bottomrule
\end{tabular}

\vspace{3pt}
\raggedright
\scriptsize $^{\mathrm{a}}$ Model buffer only, reported by \texttt{load\_tensors}. Global Q8\_0 includes vocabulary embedding. \\
...
\begin{tabular}{ccccc}
\toprule
\textbf{Context} & \textbf{TTFT} & \textbf{TPOT} & \textbf{Decode} \\
\textbf{Length} & \textbf{(ms)} & \textbf{(ms/tok)} & \textbf{(tok/s)} \\
\midrule
...
We presented MiniLlama-Infra, a from-scratch LLM inference engine designed as a micro-architecture ablation framework for edge GPU deployment. Through a five-stage optimization pipeline profiled with NVIDIA Nsight Compute, we demonstrated three key findings: (1)~naive INT8 quantization is counterproductive on consumer GPUs due to uncoalesced memory access, with throughput dropping from 58.1 to 19.2~GB/s---a result that underscores the primacy of access pattern optimization over data compression; (2)~warp-level cooperative dequantization is the single most critical optimization, alone restoring throughput to 85.6~GB/s and doubling end-to-end decoding speed; (3)~once the memory bottleneck is resolved, the system enters a compute-bound regime where further fusion yields diminishing returns. Our hybrid-precision approach achieves 35.2~tok/s at 2054~MB VRAM with a WikiText-2 perplexity of 11.52---0.28 PPL lower than llama.cpp's global Q8\_0---providing a reproducible reference for the hardware-level optimization landscape of edge LLM inference. Future work includes implementing batched GEMM-based prefill and INT8 dot-product instructions (\texttt{dp4a}) to further close the throughput gap with production frameworks.
