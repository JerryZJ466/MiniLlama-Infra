...
\begin{figure}[htbp]
    ...
    \caption{Memory access patterns for Stage~2 (Naive INT8, left) vs.\ Stage~3 (Warp-Opt INT8, right). In Stage~2, each thread in a warp independently reads a different row of the weight matrix, issuing 32 scattered cache-line requests (stride = row size). In Stage~3, all 32 threads cooperatively process one row, reading 128 consecutive bytes in a single coalesced transaction. This restructuring is the sole cause of the 5.7$\times$ kernel speedup.}
...
\begin{figure}[htbp]
    ...
    \caption{Extended roofline model on RTX~4060 Laptop. Stages~1--5 remain memory-bandwidth-bound; Stage~6 dp4a W8A8 crosses into the compute-bound region (IPC: 1.43$\to$2.08, memory BW utilization: 31.5\%$\to$1.19\%), confirming the micro-architectural regime shift.}
    \label{fig:roofline}
\end{figure}
