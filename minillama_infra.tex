...
In this work, we present \textbf{MiniLlama-Infra}, a transparent, from-scratch inference engine for Llama-3.2-1B designed explicitly as a \emph{micro-architecture ablation framework}. Rather than pursuing state-of-the-art throughput, our goal is to provide a fully traceable optimization roadmap where each technique's contribution can be precisely measured through hardware profiling. Our approach involves building the engine incrementally---from a naive FP32 baseline to an optimized INT8 pipeline with fused attention---and profiling each variant with NVIDIA Nsight Compute to extract kernel-level metrics including occupancy, memory throughput, IPC, and cache hit rates.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\columnwidth, trim=0 120 0 120, clip]{Fig0_Architecture_v2.pdf}
    \caption{System architecture of MiniLlama-Infra. Colored modules indicate optimization points: warp-level cooperative INT8 GEMV (red), fused attention in shared memory (green), and fused Add+RMSNorm (blue). The decode loop streams token IDs from host to device and returns logits for sampling.}
...
We construct a five-stage ablation study, profiling each stage with NVIDIA Nsight Compute (full metric set, single-kernel capture) on an RTX~4060 Laptop GPU. All profiling targets the GEMV kernel for a single Transformer layer with $d=2048$. Table~\ref{tab:nsight} summarizes the key hardware metrics, and Fig.~\ref{fig:memory_wall} visualizes the end-to-end impact of each stage.
...
The L2 cache hit rate drops to 2.63\%, and end-to-end throughput barely improves to 16.1~tok/s. This result is significant: it demonstrates that \emph{on consumer GPUs with limited L2 capacity, naive quantization can negate the bandwidth savings entirely}, a finding that has not been quantitatively documented with hardware profiling in prior work.
...
\begin{table}[htbp]
\centering
\caption{End-to-End Ablation: Throughput, Quality, and Energy Efficiency (RTX~4060 Laptop)}
...
\begin{table}[htbp]
\centering
\caption{Nsight Compute Profiling of the GEMV Kernel (RTX 4060, $d=2048$)}
\label{tab:nsight}
\setlength{\tabcolsep}{2.5pt}
\scriptsize
\begin{tabular}{lccc}
\toprule
\textbf{Metric} & \textbf{FP32} & \textbf{Naive INT8} & \textbf{Warp-Opt INT8} \\
\midrule
Duration ($\mu$s) & 289.1 & 298.5 & \textbf{52.3} \\
Achieved Occupancy (\%) & 16.6 & 16.6 & \textbf{86.8} \\
Mem.\ Throughput (GB/s) & 58.1 & 19.2 & \textbf{85.6} \\
Executed IPC & 0.11 & 0.18 & \textbf{1.43} \\
L2 Hit Rate (\%) & 0.86 & 2.63 & \textbf{7.80} \\
No Eligible Warps (\%) & 97.3 & 95.5 & \textbf{63.8} \\
Grid Size & 8 & 8 & \textbf{256} \\
Total Threads & 2,048 & 2,048 & \textbf{65,536} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
    ...
    \caption{Five-stage ablation: MatMul memory throughput (bars) and end-to-end decoding speed (line). Naive INT8 achieves \emph{lower} throughput than FP32 due to uncoalesced access. Warp-level optimization restores bandwidth. Fused attention adds 13\%.}
...
\begin{figure}[htbp]
    ...
    \caption{Roofline model on RTX 4060. The Warp-Opt INT8 kernel approaches the memory bandwidth ceiling; the Naive INT8 kernel severely underutilizes available bandwidth despite smaller data.}
    \label{fig:roofline}
\end{figure}
...
We select Llama-3.2-1B as the evaluation target for two reasons. First, it is representative of the class of edge-deployable LLMs that fit within 8~GB VRAM at INT8 precision while retaining competitive language modeling quality. Second, our ablation methodology requires repeated Nsight Compute profiling with full metric collection (44 replay passes per kernel capture), which imposes substantial overhead. The 1B model provides a practical balance between model complexity and profiling feasibility; larger models (3B, 7B) would require multi-hour profiling sessions per ablation stage, limiting experimental iteration. We note that the micro-architectural findings (coalescing behavior, occupancy scaling, warp stall distributions) are architecture-dependent rather than model-size-dependent and are expected to generalize to larger models on the same GPU.

We compare three configurations: (1)~PyTorch (HuggingFace Transformers) with FP16 weights, (2)~llama.cpp with Q8\_0 global quantization, and (3)~MiniLlama-Infra with INT8 group-64 quantization and FP32 vocabulary. Decoding throughput is measured over 128 generated tokens with greedy sampling (temperature~0) and averaged across 4 prompts. Perplexity is evaluated on the WikiText-2 test set~\cite{merity2016wikitext} with a sliding window of 2048 tokens and stride 512.

\subsection{End-to-End Performance}
Table~\ref{tab:end_to_end} presents the end-to-end comparison. MiniLlama-Infra achieves 35.2~tok/s decoding throughput at 2054~MB peak VRAM. Compared to PyTorch, our engine delivers a 13.6\% VRAM reduction while maintaining comparable perplexity (11.52 vs.\ 11.50).

The throughput gap relative to llama.cpp (144.9~tok/s) reflects the accumulated optimizations in that framework, including tiled GEMM for batched prefill, vectorized INT8 dot-product instructions (\texttt{dp4a}), fused multi-head attention kernels, and two years of community-driven optimization. Our single-developer, from-scratch engine does not incorporate these advanced techniques. However, our hybrid-precision approach achieves a measurably lower perplexity (11.52 vs.\ 11.80), demonstrating that the VRAM cost of preserving FP32 vocabulary precision is justified by the accuracy benefit.
...
\midrule
\textbf{Ours} & \textbf{INT8+FP32$^{\mathrm{b}}$} & \textbf{2054} & \textbf{35.2} & \textbf{11.52} \\
\bottomrule
\end{tabular}
...
\scriptsize $^{\mathrm{b}}$ INT8 group-64 for hidden layers; FP32 for vocabulary embedding and classification head.
\end{table}

\subsection{Accuracy Verification}
We evaluate perplexity on the WikiText-2 test set using a sliding window approach. To isolate the effect of quantization from the inference engine implementation, we apply simulated INT8 group-64 quantization to the HuggingFace model weights (fake quantization: round-trip through INT8 and back to FP16, with \texttt{lm\_head} excluded). The FP16 baseline achieves a perplexity of 11.50, while the INT8 group-64 variant achieves 11.52---a degradation of only 0.02 PPL points. In contrast, llama.cpp's global Q8\_0 quantization achieves 11.80---a 0.30 PPL degradation from the FP16 baseline.
...
\begin{figure}[htbp]
    ...
    \caption{TTFT scaling with context length. The linear growth confirms the sequential forward pass bottleneck of single-query GEMV-based prefill, producing $O(N^2)$ total compute.}
    \label{fig:context_scaling}
\end{figure}

\subsection{Stage 6: dp4a W8A8 Integer SIMD}

\textbf{Finding: Full W8A8 quantization via \texttt{\_\_dp4a} delivers a 25\% throughput gain with less than 1.3\% quality degradation.}
...
Our implementation quantizes activations dynamically per-token using a warp-level reduction with the float-as-uint \texttt{atomicMax} trick, then dispatches the \texttt{\_\_dp4a} GEMV with per-group INT32 accumulation before rescaling to FP32.

End-to-end decoding throughput increases from 35.2~tok/s (Stage~5) to \textbf{44.0~tok/s} ($+$25\%, mean of four runs: 45.56, 42.33, 42.22, 43.70~tok/s).
To verify quality, we evaluate perplexity on WikiText-2 using a simulated fake-quantization model in PyTorch (Table~\ref{tab:ablation_summary}).
W8A8 raises PPL by only 0.16 points ($+$1.2\% relative) versus the FP16 baseline, while W8A32 raises it by just 0.02 points.
This confirms that per-token dynamic activation quantization preserves model quality at a level suitable for inference deployment.

Nsight Compute profiling of the \texttt{matmul\_dp4a\_kernel} reveals a qualitative regime shift relative to Stage~3.
Occupancy remains at 86.84\% (identical to Stage~3), confirming that the warp-level parallelism structure is preserved.
However, IPC rises from 1.43 to \textbf{2.08} ($+$45\%), a direct consequence of \texttt{\_\_dp4a} replacing four scalar multiplications with a single fused instruction.
Most significantly, global memory bandwidth utilization drops from 31.5\% to \textbf{1.19\%} of peak---a 96\% reduction---indicating that the kernel has transitioned from a \emph{memory-bandwidth-bound} regime (Stage~3) to a \emph{compute-bound} regime (Stage~6).
This phase transition is the hardware-level explanation for the throughput gain: once the memory wall is no longer the bottleneck, the additional arithmetic density of \texttt{\_\_dp4a} translates directly into higher throughput.

We further measure energy efficiency using \texttt{pynvml} power sampling at 50~ms intervals during inference.
The RTX~4060 Laptop draws a stable 76.7~W (min/max: 72.6/81.0~W) across both stages.
Stage~5 achieves 0.446~tok/J, while Stage~6 achieves \textbf{0.584~tok/J}---a \textbf{31\% improvement in energy efficiency} at identical GPU power draw.
This result demonstrates that \texttt{\_\_dp4a} delivers more useful work per joule, not merely faster execution at higher power, making it particularly attractive for battery-constrained or thermally-limited edge deployments.

\subsection{Cross-Architecture Validation}

To assess whether our findings generalize beyond Ada Lovelace, we reproduced the three-stage kernel profiling on two additional GPU generations using Ubuntu VM instances with full hardware-counter access: RTX~3090 (Ampere, SM~8.6, 936~GB/s peak) and RTX~2080~Ti (Turing, SM~7.5, 616~GB/s peak). Table~\ref{tab:cross_arch} summarizes the results.

The qualitative trend is fully consistent across all three architectures. Stages~1 and~2 exhibit nearly identical occupancy and IPC on every GPU (Stage~1 occupancy: 16.6\%\,/\,16.6\%\,/\,25.0\%; IPC: 0.11\,/\,0.11\,/\,0.10), confirming that the low-occupancy pathology of single-threaded GEMV is architecture-independent. Stage~3 dramatically improves both metrics on all three GPUs: occupancy jumps to 86.8\%\,/\,49.4\%\,/\,93.6\% and IPC rises to 1.43\,/\,0.95\,/\,1.32 for the 4060\,/\,3090\,/\,2080~Ti, respectively.

The lower absolute occupancy on the RTX~3090 (49.4\% vs.\ 86.8\%) reflects its larger SM count (82 vs.\ 24): identical grid dimensions spread fewer warps per SM, reducing theoretical occupancy without degrading the memory coalescing benefit. End-to-end throughput scales broadly with memory bandwidth (31.8\,/\,56.4\,/\,27.0~tok/s), consistent with the memory-bound characterization established in Section~IV.

These results confirm that \emph{warp-level memory coalescing is the dominant optimization lever across Turing, Ampere, and Ada Lovelace GPU generations}, validating the generalizability of our central finding.

\begin{table}[htbp]
\centering
\caption{Cross-Architecture GEMV Kernel Profiling ($d=2048$)}
...
\textbf{Stage} & \textbf{Metric} & \textbf{RTX 4060} & \textbf{RTX 3090} & \textbf{RTX 2080 Ti} \\
 & & \textit{Ada SM~8.9} & \textit{Ampere SM~8.6} & \textit{Turing SM~7.5} \\
\midrule
\multirow{2}{*}{Stage 1 (FP32)} & Occupancy (\%) & 16.6 & 16.6 & 25.0 \\
 & IPC & 0.11 & 0.11 & 0.10 \\
\midrule
\multirow{2}{*}{Stage 2 (Naive INT8)} & Occupancy (\%) & 16.6 & 16.6 & 25.0 \\
 & IPC & 0.18 & 0.18 & 0.18 \\
\midrule
\multirow{3}{*}{Stage 3 (Warp-Opt)} & Occupancy (\%) & \textbf{86.8} & \textbf{49.4} & \textbf{93.6} \\
...
Third, our INT8 dequantization uses scalar multiply-accumulate operations. Modern NVIDIA GPUs provide \texttt{dp4a} instructions that perform four INT8 multiplications in a single cycle. Integrating \texttt{dp4a} into the warp-level kernel would further improve arithmetic throughput.

Finally, our context scaling analysis reveals severe prefill latency at longer sequences, motivating the adoption of FlashAttention-style tiled attention with online softmax for future versions of the engine.

%==========================================================
\section{Conclusion}
%==========================================================

We presented MiniLlama-Infra, a from-scratch LLM inference engine designed as a micro-architecture ablation framework for edge GPU deployment. Through a five-stage optimization pipeline profiled with NVIDIA Nsight Compute, we demonstrated three key findings: (1)~naive INT8 quantization is counterproductive on consumer GPUs due to uncoalesced memory access, with throughput dropping from 58.1 to 19.2~GB/s---underscoring the primacy of access pattern optimization over data compression; (2)~warp-level cooperative dequantization is the single most critical optimization, alone restoring throughput to 85.6~GB/s and doubling end-to-end decoding speed; (3)~once the memory bottleneck is resolved, the system enters a compute-bound regime where further fusion yields diminishing returns. Our hybrid-precision approach achieves 35.2~tok/s at 2054~MB VRAM with a WikiText-2 perplexity of 11.52---0.28 PPL lower than llama.cpp's global Q8\_0---providing a reproducible reference for the hardware-level optimization landscape of edge LLM inference. Future work includes batched GEMM-based prefill and \texttt{dp4a} instructions to further close the throughput gap with production frameworks.The source code and profiling data will be made publicly available upon acceptance.
