...
\begin{itemize}
    ...
    \item \textbf{dp4a W8A8 as a Compute-Regime Enabler:} Full W8A8 quantization via \texttt{\_\_dp4a} shifts the GEMV kernel from memory-bandwidth-bound to compute-bound (memory BW utilization: 31.5\%$\to$1.19\%; IPC: 1.43$\to$2.08), delivering a further 25\% throughput gain and 31\% energy efficiency improvement at identical GPU power.
    \item \textbf{Cross-Scale Generalization:} Our optimizations generalize to the 3B model on RTX~3090, achieving a 4.34$\times$ cumulative speedup (FP32: 8.05~tok/s $\to$ dp4a W8A8: 35.07~tok/s), with the Stage~5$\to$6 gain (1.22$\times$) consistent with the 1B result (1.25$\times$).
\end{itemize}
